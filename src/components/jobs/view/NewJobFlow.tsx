...
// Managed job worktrees live under <repo>/.gjc-worktrees/<jobId>; offering one
// as a job target would nest worktrees. Segment match keeps this portable
// across separators instead of assuming the POSIX spelling.
...
export default function NewJobFlow({ projects, onCreated }: { projects: Project[]; onCreated: (jobId: string) => void }) {
  ...
  // Reconcile the controlled value with list membership: projects load
  // asynchronously, and a previously selected path can disappear.
  useEffect(() => {
  ...
  const create = async (event: React.FormEvent) => {
    ...
    if (!selectableProjects.some((project) => project.fullPath === projectPath)) {
  ...
  return <main className="flex flex-1 items-center justify-center p-6"><form onSubmit={create} className="w-full max-w-xl space-y-4 rounded-lg border p-5">
    ...
    <select className="w-full rounded border p-2" value={projectPath} onChange={e => setProjectPath(e.target.value)} required><option value="" disabled>Select a project</option>{selectableProjects.map(project => <option key={project.projectId} value={project.fullPath}>{project.displayName}</option>)}</select>
    {selectableProjects.length === 0 && <p className="text-sm text-muted-foreground">No selectable projects yet — job worktrees are excluded. Create or open a project first.</p>}
    ...
    <p className="text-sm text-muted-foreground">Permission mode: default</p>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}<button className="rounded bg-primary px-4 py-2 text-primary-foreground" type="submit" disabled={selectableProjects.length === 0}>Start job</button>
  </form></main>;
}
