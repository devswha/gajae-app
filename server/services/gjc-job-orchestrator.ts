...
export class JobOrchestrator {
  ...
  private async failRun(jobId: string, runId: string, expected: Lease, abortHandle: string | undefined, error: unknown): Promise<void> {
    ...
    const fresh = snapshot(await this.deps.jobs.get({ jobId }));
    if (sameFence(fresh, runId, expected)) await this.finalize(jobId, fresh, runId, 'failed', { kind: 'persistence_failed', error: error instanceof Error ? error.message : 'Durable job persistence failed.' });
  ...
  private async dispatch(jobId: string, current: JobSnapshot, runId: string, appSessionId: string, message: string, options: JobOrchestratorOptions, cwd: string, sessionId?: string | null): Promise<JobRunHandle> {
    ...
    try {
      ...
      const persistenceFailure = (error: unknown) => {
        void this.serial(jobId, () => this.failRun(jobId, runId, expected!, run?.abortHandle, error)).catch(() => undefined);
      };
  ...
  async start(provider: 'gjc', appSessionId: string, projectRoot: string, message: string, options: JobOrchestratorOptions): Promise<JobRunHandle> {
    ...
    return this.serial(jobId, async () => {
      ...
      try {
        ...
        return this.dispatch(jobId, current, runId, appSessionId, message, options, created.path);
      } catch (error) {
        await this.deps.jobs.transition({ ...this.params(jobId, current), state: 'failed' }).catch(() => undefined);
        throw error;
      }
    });
  }
  async turnStart(provider: 'gjc', appSessionId: string, message: string, options: JobOrchestratorOptions): Promise<JobRunHandle> {
    ...
    return this.serial(bound.jobId, async () => {
      ...
      try {
        ...
        return this.dispatch(bound.jobId, current, runId, appSessionId, message, options, cwd, bound.providerSessionId);
      } catch (error) {
        await this.failRun(bound.jobId, runId, lease(current), undefined, error).catch(() => undefined);
        throw error;
      }
    });
  }
  async resume(jobId: string, appSessionId: string, message: string, options: JobOrchestratorOptions): Promise<JobRunHandle> {
    ...
    return this.serial(jobId, async () => {
      ...
      try {
        return this.dispatch(jobId, admitted, runId, appSessionId, message, options, cwd, bound.providerSessionId);
      } catch (error) {
        await this.failRun(jobId, runId, lease(admitted), undefined, error).catch(() => undefined);
        throw error;
      }
    });
  }
...
export function getProductionJobOrchestrator(): ProductionOrchestrator {
  ...
  let orchestrator: ProductionOrchestrator | undefined;
  ...
  orchestrator = new JobOrchestrator({ jobs, gitForProject, supervisor: getGjcWorkerSupervisor() }) as ProductionOrchestrator;
